...
class VideoStitcher:
    def __init__(self, label_divisor) -> None:
        ...
        self._label_divisor = self.frame_stitcher._label_divisor
        self._overlap_offset = self.frame_stitcher._overlap_offset
    ...
    def reset(self):
        ...
        self._next_panoptic = None
        self._last_panoptic = None

    def infer(self, panoptic, panoptic2):
        if self._next_panoptic is None:
            self._stitched_panoptic.append(panoptic)
            self._next_panoptic = tf.expand_dims(panoptic2, axis=0)
            return 
        
        self._next_panoptic = self.frame_stitcher(self._next_panoptic, tf.expand_dims(panoptic, axis=0))
        next_panoptic = tf.squeeze(self._next_panoptic).numpy()
...
def main(saved_predictions_root, output_dir_root):
    tf.io.gfile.makedirs(output_dir_root)
    physical_devices = tf.config.list_physical_devices('GPU')
    tf.config.set_visible_devices(devices=physical_devices[1:], device_type='GPU')
    ...
    # context_names = list(context_names)[:1] # for debugging
